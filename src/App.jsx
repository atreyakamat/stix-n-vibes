...
const CustomCollabs = lazy(() => import('./pages/CustomCollabs'));
const NotFound = lazy(() => import('./pages/NotFound'));

// Premium, brand-aligned transition loader
function Loader() {
  return (
    <div className="min-h-screen bg-beige text-gray-900 flex items-center justify-center font-mono text-[10px] sm:text-xs tracking-[0.3em] uppercase select-none">
      <div className="flex flex-col items-center gap-6">
        <div className="relative w-12 h-12 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/5 rounded-full border border-black/10 animate-ping" />
          <span className="text-gray-900 text-xl animate-pulse">✦</span>
        </div>
        <span className="animate-pulse text-gray-400">PREPPING_THE_VIBE...</span>
      </div>
    </div>
  );
}
    ...
    <Router>
      ...
      <Suspense fallback={<Loader />}>
        <Routes>
          ...
          <Route path="/collabs" element={<CustomCollabs />} />
